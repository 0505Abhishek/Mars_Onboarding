...
const approveLeadView = async(req,res)=>{
      try {
          ...
          let allDistributor = await approveLead.getDistributorList(req.cookies.role_id, req.cookies.region_id, req.cookies.role_id);
...
const approveLeadViewById = async(req,res)=>{
    ...
    let data = await dashboard.selectQuery(req.cookies.email);
    let navbarviews = await dashboard.navbarviewesult(data);
    let distributor = await approveLead.getDistributorById(req.params.id, req.cookies.role_id, req.cookies.region_id);
...
const Rsem_approval = async(req,res)=>{
   ...
   let distributor = await approveLead.getDistributorById(id, req.cookies.role_id, req.cookies.region_id);
   ...
   if ([2, 3].includes(distributor[0]?.flag)) {
    ...
    if(action==="Approved")
    { 
        await editDistributor.updateDistributorById(1,action, 1, id, req.cookies.role_id, 1, remark);
        await approveLead.updateProspectiveForRsem(1,id,remark);

        let emailObj= {sendToEmail: distributorDetail[0].aseemail, sendToName: userDetail.employee_name, sendByName: UserName, firmName: distributorDetail[0].firmName, SendToRole: userDetail.role, remark:remark};
        ...
        let distributor = await editDistributor.updateDistributorById(0,action, 2, id, req.cookies.role_id, 1, remark);
        ...
        await approveLead.updateProspectiveInfo(action, 3,'RSEM', req.cookies.user_id, id, remark);
...
module.exports = {approveLeadView,approveLeadViewById, Rsem_approval, TerritoryByRegion};