...
const dbManagment = require("../../models/super_dbmanagment/offdbManagment.model");
const fs = require("fs");
...
const offDbManagmentPage = async (req, res, next) => {
  try {
    ...
    res.render("offdbmanagment/index", {
      token: navbarviews,
...
const OffEditDbManagmentPage = async (req, res, next) => {
  try {
    let email = decryptData(req.cookies.e);
    let data = await navbar.selectQuery(email);
    let navbarviews = await navbar.navbarviewesult(data);
    ...
    if (!getdbdataById) {
      ...
      return res.redirect("/offdbmanagment/index");
    }

    let getdbdataByIdApproval =
      await dbManagment.getdataByIdApprovalOffboarding(id);

    let territory = await dbManagment.getterritory();
    res.render("offdbmanagment/offedit", {
      token: navbarviews,
      user: res.userDetail,
      success: req.session.success,
      error: req.session.error,
      getuserdata: getuserdata,
      getdbdataById: getdbdataById,
      getdbdataByIdApproval: getdbdataByIdApproval,
      territory: territory,
    });
    req.session.destroy();
  } catch (error) {
...
const offboardingApproverupdate = async (req, res, next) => {
  try {
    const data = req.body;
    let loginsert = await dbManagment.getdataByIdApprovalOffboarding(
      data.application_id
    );

    const logInsertPromises = loginsert.map((row) =>
      dbManagment.insertApprovalLogoffboard(row)
    );
...
const approverupdate = async (req, res, next) => {
  try {
    ...
    let approverrole_id = data.approver_id[currentIndex];
    ...
    const logInsertPromises = loginsert.map((row) =>
      dbManagment.insertApprovalLog(row)
    );
    await Promise.all(logInsertPromises);
    ...
    if (approverrole_id == 1) {
      ...
      if (users.length > 0) {
        await dbManagment.updateDBStatus(
          data.application_id,
          users[0].id,
          users[0].email_id,
          data.territory
    ...
    req.session.success = "Approver updated successfully";
    return res.redirect("/off_db_managment");
  } catch (e) {
    console.log("error:- ", e);
    req.session.error = "Failed to update approver";
    return res.redirect("/off_db_managment");
...
const upload = async (req, res, next) => {
  try {
    ...
    if (!headersMatch) {
      req.session.error =
        "CSV headers do not match the expected format. Please check the file and try again.";
      return res.redirect("/db_managment");
    }
    ...
    for await (const item of csvData) {
      ...
      let checkterritory_id = await dbManagment.getterritory_id(
        mappedItem.territory_id
      );
      mappedItem.territory_id = checkterritory_id.id;
      mappedItem.region_id = checkterritory_id.region_id;
      let region_id = await dbManagment.getregion_id(
        checkterritory_id.region_id
      );
      ...
      if (!usercheck) {
        try {
          ...
          if (results) {
            ...
            let document = await dbManagment.insertdbdocument(
              application_id,
              mappedItem
            );
            let createDistributors = await createDistributor(
              application_id,
              mappedItem,
              region_name
            );
...
const updateTerritory = async (req, res) => {
  try {
    ...
    const csvFile = req.files.file;
    let csvData = await processCSV(csvFile.path);
    const expectedHeaders = ["Territory Name", "Distributor ID"];
    ...
    for await (const item of csvData) {
      ...
      let checkuser_id = await dbManagment.getdbdataById(
        item["Distributor ID"]
      );
      if (!checkuser_id) {
      ...
      if (checkterritory_id && checkuser_id) {
        let createDistributors = await updateDistributorterritory(
          checkterritory_id.id,
          checkuser_id,
          region_name
        );
        ...
        if (createDistributors == true) {
          ...
          let updateProspectiveInfos =
            await dbManagment.updateProspectiveInfoasm(
              checkuser_id.id,
              dataupdate
            );
...
const updateDistributorterritory = async (territory_ids, item, region_name) => {
  try {
    ...
    let hierarchyPersons = await dbManagment.getAllhierarchyPersons(
      territory_id
    );
    ...
    for (let role of approvalHierarchy) {
      ...
      if (
        role === "ASM" &&
        hierarchy.find((h) => h.role === "ASM" && h.user_id === person.user_id)
      ) {
        continue;
      }
    ...
    let updateProspectiveInfo = await dbManagment.updateProspectiveInfo(
      hierarchy,
      hierarchy.length,
      hierarchy[0],
      lastApprover,
...
const createDistributor222 = async (application_id, item) => {
  try {
    ...
    for (let i = 0; currentUserId && i < maxIterations; i++) {
      ...
      if (!user) {
        console.warn(
          `Warning: User with ID ${currentUserId} not found. Breaking loop.`
        );
        break;
      }
    ...
    let lastApprover = hierarchy?.[hierarchy.length - 1] || null;
    await dbManagment.updateProspectiveInfo(
      hierarchy,
      hierarchy.length,
      hierarchy[0],
      lastApprover,
...
const exportDatabase = async (req, res) => {
  try {
    ...
    let approvalWorkflow = await dbManagment.getApprovalWorkflow(
      getdbdata.map((item) => item.pid)
    );
    let regionandterritory = await dbManagment.regionandterritorybyid(
      getdbdata.map((item) => item.territory_id)
    );
    let roleInfo = await dbManagment.getroleInfo();

    const uniqueApproverIds = [
      ...new Set(approvalWorkflow.map((item) => item.approver_id)),
    ];
    let users = (
      await Promise.all(
    ...
    if (offboardingdata && offboardingdata.length > 0) {
      approvalWorkflowoffboarding =
        await dbManagment.getApprovalWorkflowoffboarding(
          offboardingdata.map((item) => item.pid)
        );
      regionandterritoryoffboarding = await dbManagment.regionandterritorybyid(
        offboardingdata.map((item) => item.territory_id)
      );
      const uniqueApproverIdsOffboarding = [
        ...new Set(approvalWorkflowoffboarding.map((item) => item.approver_id)),
      ];
      usersOffboarding = (
        await Promise.all(
    ...
    const combinedData = [
      ...getdbdata.map((item) => {
        const itemWorkflows = approvalWorkflow.filter(
          (workflow) => workflow.application_id === item.id
        );
        ...
        return {
          ...
          ...Object.fromEntries(
            Array.from(workflowRoles).map((role, index) => {
              const matchingWorkflow = approvalInfos.find(
                (info) => info.role === role
              );
              return [
                `approvalWorkflow_${index}`,
                matchingWorkflow ? matchingWorkflow.name : "N/A",
              ];
            })
          ),
          approvalWorkflow: approvalInfos
            .map((info) => `${info.role}: ${info.name}`)
            .join(" | "),
        };
      }),

      ...offboardingdata.map((item) => {
        const itemWorkflows = approvalWorkflowoffboarding.filter(
          (workflow) => workflow.application_id === item.id
        );

        const approvalInfos = itemWorkflows.map((workflow) => {
          const user = usersOffboarding.find(
            (u) => u.id === workflow.approver_id
          );
          const roleInfo = {
            role: workflow.role_name || "N/A",
            name: user ? user.employee_name || "N/A" : "N/A",
