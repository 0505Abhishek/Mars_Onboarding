...
const selectQuery = async (data) => {
    const query = `SELECT * FROM s_tbl_user WHERE email_id = ? AND user_type = 'SUPERADMIN' AND flag = 1`;
    
    return new Promise((resolve, reject) => {
        mysqlConnection.query(query, [data.email], (error, result) => {
            if (error) {
                return reject(error);
            }
            resolve(result[0]); // Or resolve(result); if you want full array
        });
    });
};
...
const selectQueryById = async (data) => {
    let query = `SELECT * FROM s_tbl_user WHERE id = '${data.id}'`;
...
const updateQuery = async (data) => {
    ...
    return new Promise((resolve, reject) => {
        let query = `UPDATE s_tbl_user SET ? WHERE email_id =?`;
...
const updatePasswordByToken = async (data, password) => {
    ...
    return new Promise((resolve, reject) => {
        let query = `UPDATE s_tbl_user SET password = ?, password_last_updated = NOW() WHERE email_id = ?`;
...
const setLoginCount = async (count, email) => {
    ...
    return new Promise((resolve, reject) => {
        const query = `UPDATE s_tbl_user SET login_count = ?, system_logged_time = ? WHERE email_id = ?`;
...
module.exports = { setLoginCount, selectQueryById, insertQuery, selectQuery, selectQueryByToken, updateQuery, updateQueryByToken, updateQueryPassword, updatePasswordByToken, getPassworHistory, password_history, updateLockout, updateLoginAttempt }