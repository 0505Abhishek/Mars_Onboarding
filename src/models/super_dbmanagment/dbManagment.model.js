...
const getdata = async () => {
  try {
    let query = `SELECT tbl_approval_workflow.cdCode, prospective_info.*, prospective_info.id as pid FROM prospective_info
                 left join tbl_approval_workflow on tbl_approval_workflow.application_id = prospective_info.id
                 WHERE  1 = 1 and  tbl_approval_workflow.approver_role_id = 8 Order By prospective_info.create_date Desc`;
...
const getdataByIdApproval = async (id) => {
  try {
    let query = `SELECT * FROM tbl_approval_workflow WHERE application_id = ${id}`;
    return new Promise((resolve, reject) => {
      dbconn.query(query, (error, results) => {
        if (error) {
          console.error("Database error:", error);
...
const approverupdate = async (data) => {
  try {
    ...
    for (let i = 0; i < data.approver_id.length; i++) {
      ...
      const sequence = data.approval_sequence[i];
      const query = `UPDATE tbl_approval_workflow SET approver_id = ?, territory_id = ? WHERE application_id = ? AND approval_sequence = ?`;
      promises.push(
...
const insertApprovalLog = async (log) => {
  try {
    const query = `INSERT INTO s_tbl_approval_workflow_log SET ? `;
    return new Promise((resolve, reject) => {
...
const getApproversByTerritory = async (territoryId) => {
  try {
    const query = `
            ...
            AND r.role_id IN (1,2,3,4,5,6,7,8,9)
            ORDER BY FIELD(r.role_id, 1,2,3,4,5,6,7,8,9)`;
