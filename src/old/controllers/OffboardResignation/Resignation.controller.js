...
const offboardList = async (req, res) => {
  try{
        ...
        let allDistributor = await ResignationModel.getDistributorList(req.cookies.role_id, req.cookies.region_id);
...
const offboardResignationViewById = async(req,res)=>{
  ...
  let Distributor = await ResignationModel.getDistributorByID(application_id, req.cookies.role_id, req.cookies.region_id);
...
const SubmitResignation = async (req, res) => {
  try {
    ...
    const Distributor = await ResignationModel.getDistributorByID(actionData.applicationId, req.cookies.role_id, req.cookies.region_id);
    ...
    if (actionData.action === 'Approve') {
      await ResignationModel.updateoffboardHierarchy(user_id, actionData.applicationId, 1, actionData.remarkMsg, req.cookies.role_id);
      await ResignationModel.updateoOffboardingDistributorApproved(actionData.applicationId);
      const offboardHierarchyData = await ResignationModel.getoffboardHierarchy(user_id, actionData.applicationId, req.cookies.role_id);
    ...
    if (actionData.action === 'Reject') {
      await ResignationModel.updateRejectoffboardHierarchy(user_id, actionData.applicationId, actionData.remarkMsg, req.cookies.role_id);
