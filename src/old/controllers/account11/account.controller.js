...
const { insertQuery, selectQueryById, selectQuery, updateQuery, updateQueryByToken, updatePasswordByToken, password_history, getPassworHistory, setLoginCount} = require("../../models/account.model");
...
const userLogin = async (req, res) => {
    try {
        ...
        if (loginCount >= 25 && timeDiffInMinutes < 30) {
            ...
            await setLoginCount(loginCountContainer, results.email_id); 
        }
        ...
        if (password !== user.password) {
            loginCountContainer++;
           let sac= await setLoginCount(loginCountContainer, user.email_id);
...
const userLogout = async (req, res) => {
    ...
    try {
        ...
        res.clearCookie("t", { httpOnly: true })
        res.clearCookie("e", { httpOnly: true });
        res.clearCookie("u_d", { httpOnly: true });
        res.clearCookie("r_n", { httpOnly: true });
        res.clearCookie("u_n", { httpOnly: true });
...
const createPasswordView = async (req, res) => {
    try {
        return res.render('create_password',{message: req.session.message, notification: req.session.message})
    } catch (error) {
        console.log(error);
...
const changePasswordView = async (req, res) => {
    try {
        ...
        return res.render('forgot-password',{token, message: req.session.message, notification: req.session.message})
    } catch (error) {
        console.log(error);
        return res.redirect('/account')
    }
