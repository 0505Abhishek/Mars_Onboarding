...
} = require("../../models/SaveApplicationHistory/SaveApplicationHistory.model");

const territory = async (req, res) => {
  const regionId = req.body.regionId;
  try {
    const territories = await addDistributor.getTerritory(
      req.cookies.user_id,
      regionId
    );
...
const soTerritory = async (req, res) => {
  ...
  try {
    const territories = await addDistributor.getSoTerritory(
      req.cookies.user_id,
      regionId,
      territoryId
    );
...
const addDistributorView = async (req, res, next) => {
  ...
  let hierarchyPersons = await addDistributor.getAllhierarchyPersons(
    req.cookies.user_id
  );
...
const createDistributor = async (req, res) => {
  try {
    ...
    let MarsId = await generateMarsId(
      RegionName.region_name,
      resultNewDistributor.insertId
    );

    await addDistributor.addMarsIdForDistributor(
      resultNewDistributor.insertId,
      MarsId
    );

    let distributor1 = await addDistributor.getDistributorByEmail(
      resultNewDistributor.insertId
    );
    ...
    await addDistributor.updateProspectiveInfo(
      hierarchy,
      hierarchy.length,
      hierarchy[0],
      lastApprover,
    ...
    let distributorDetail = await addDistributor.getDistributorByEmail(
      resultNewDistributor.insertId
    );

    let AllEmailsToSend = await addDistributor.getAllEmailsToSend(
      hierarchyData.role_id,
      region_id
    );
    ...
    let emailObj = {
      sendToEmail: emailList,
      sendToName: hierarchyData.employee_name,
      sendByName: UserName,
      firmName: distributorDetail[0].firmName,
      SendToRole: hierarchyData.role,
    };
    ...
    await insertApplicationHistory(
      ...
      hierarchyData.id,
      hierarchyData.role,
      hierarchyData.employee_name,
      "",
      "Submitted",
      "Distributor onboarding process initiated."
    );
    req.session.notification = {
      type: "success",
      message: "Invite sent to RSEM Successfully!",
    };
    return res.redirect("/Add_distributor");
  } catch (error) {
...
const distributorDraftList = async (req, res) => {
  try {
    let data = await dashboard.selectQuery(req.cookies.email);
    let allDistributor = await addDistributor.getDistributorDraftList(
      req.cookies.user_id,
      req,
      cookies.email
    );
    ...
    res.render("distributor_draft_list", {
      token: navbarviews,
      notification,
      data: allDistributor,
    });
...
const saveDraft = async (req, res) => {
  try {
    ...
    // Save draft
    let resultNewDistributor = await addDistributor.addDistributor(
      req.body,
      "draft"
    );

    let RegionName = await addDistributor.getTerritoryName(
    ...
    let distributor = await addDistributor.getDistributorByEmail(
      resultNewDistributor.insertId
    );
    ...
    if (!distributor.length) {
      return res
        .status(500)
        .json({ message: "Failed to retrieve distributor ID" });
    }
