...
const distributorView = async (req, res) => {
  try {
    ...
    let distributors = await correctionModel.getDistributorList(
      req.cookies.user_id
    );
...
const submitCorrection = async (req, res) => {
  try {
    ...
    const applicationData = await correctionModel.GetApplicationDetail(
      applicationId
    );
    let userdata = await correctionModel.getUserById(
      applicationData[0]?.approver_id
    );
    ...
    if (userdata?.email_id) {
      let AllEmailsToSend = await correctionModel.getAllEmailsToSend(
        userdata.role_id,
        region_id
      );
      ...
      let emailObj = {
        ...
        firmName: applicationData[0].firmName,
        SendToRole: userdata.role,
      };
