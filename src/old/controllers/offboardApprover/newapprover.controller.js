...
const submitDbReplacement = async (req, res) => {
  try {
    ...
    if (result.alreadyExists) {
      return res.json({
        success: false,
        message: "Replacement DB already submitted for this application",
      });
    }
...
const submitDbResponse = async (req, res) => {
  try {
    const { approval, amount, reason, unit, application_id } = req.body;
    // console.log('submitDbResponse called with:', req.body);
...
const initiateOffboardAsmaction = async (req, res) => {
  try {
    const { application_id, offboard_type, reason = [] } = req.body;
...
const send_email_Approve = async (approver_id) => {
...
const send_email_sac = async (approver_id) => {
...
const claimSubmit = async (req, res) => {
  try {
    ...
    if (!application_id || !type || !amount || !unit) {
      return res.status(400).json({
        type: "error",
        message: "Invalid offboarding data or user not logged in",
      });
...
const blockDbSubmit = async (req, res) => {
  try {
    ...
    if (!application_id || !blockDbRemarks || !user_id || !blockDbSelect) {
      return res.status(400).json({
        type: "error",
        message: "Invalid offboarding data or user not logged in",
      });
...
const paymentSubmitBtn = async (req, res) => {
  try {
    ...
    const row = currentRowss[0]; // Safe to access now
...
const submitGstReversal = async (req, res) => {
  try {
    ...
    await approverModel.updateWorkflowRow(row.id, {
      status: "APPROVED",
      remark: "GST Reversal form submitted",
      is_final_approval: "1",
    });
    ...
    if (nextRow) {
      ...
      if (userData?.email_id) {
        await mailer.sentEmailForOffboarding(
          {
