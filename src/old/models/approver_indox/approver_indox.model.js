  ...
  try {
    let query = `
      SELECT 
        taw.*,
        pi.*, 
        u2.employee_name as aprover_employee_name,
        pi.create_date as applicationStartDate,s_tbl_territory_master.territory_name, s_tbl_region_master.region_name,
      ...
      WHERE taw.approver_role_id = ? AND pi.type != 'EXISTING'
    `;
    ...
    if (role_id == 4) {
      ...
      query += ` AND taw.approver_role_id = ? AND pi.region_id IN (${placeholders})`;
