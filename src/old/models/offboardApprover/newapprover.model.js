...
const getDistributorList = async (role_id, region_id) => {
  ...
  let query = `
    ...
    WHERE prospective_info.offboarding_flag = ?`;
...
const getRsemPendingRow = async (application_id, approver_id) => {
  ...
  const query = `
    ...
    WHERE offboardingdistributors.application_id = ? 
      AND offboardingdistributors.applicationStatus IN ('PENDING', 'NOT_STARTED')
  ...
  return new Promise((resolve, reject) => {
    ...
    dbconn.query(checkQuery, [data.application_id], (err, rows) => {
      ...
      if (rows.length > 0) {
        return resolve({
          alreadyExists: true,
          replace_status: rows[0].replace_status,
        });
      }

      const insertQuery = `
        INSERT INTO db_replacements
        (application_id, is_replacement, replacement_db_name, replacement_db_code, action_date, replace_status)
        VALUES (?, ?, ?, ?, ?, 1)
      `;
      ...
      const params = [
        data.application_id,
        data.is_replacement,
        data.replacement_db_name,
        data.replacement_db_code,
        data.action_date,
      ];

      dbconn.query(insertQuery, params, (err2, result) => {
  ...
  try {
    ...
    if (exists) {
      ...
      return new Promise((resolve, reject) => {
        dbconn.query(
          updateQuery,
          [type, amount, unit, application_id],
          (error, results) => {
            if (error) return reject(error);
            resolve({ action: "updated", ...results });
          },
      ...
      const insertQuery = `
        INSERT INTO db_replacements (application_id,type,amount,unit)
        VALUES (?, ?, ?, ?)
      `;
      ...
      return new Promise((resolve, reject) => {
        dbconn.query(
          insertQuery,
          [application_id, type, amount, unit],
          (error, results) => {
            if (error) return reject(error);
            resolve({ action: "inserted", ...results });
          },
...
const insertDbResponse = async (data) => {
  const query = `INSERT INTO db_responses (application_id, claim_type, counter_amount, counter_unit, status) VALUES (?, ?, ?, ?, 'PENDING')`;
  return new Promise((resolve, reject) => {
    dbconn.query(
      query,
      [
...
const updateDbResponseCounterOffer = async (application_id, data) => {
  const query = `INSERT INTO db_responses SET counter_amount = ?, counter_unit = ?, counter_reason = ?, status = ? WHERE application_id = ?`;
  return new Promise((resolve, reject) => {
    dbconn.query(
      query,
      [
...
`;
      ...
      return await new Promise((resolve, reject) => {
        dbconn.query(
          updateQuery,
          [replaceStatus, replaceStatus, data.remarks, data.application_id],
          (err, result) => {
            if (err) return reject(err);
    ...
    const insertQuery = `
  INSERT INTO db_replacements
  (application_id, dblinkstatus, replacement_status, remarks_replace, replaceaction_date)
  VALUES (?, ?, ?, ?, NOW())
`;
    ...
    return await new Promise((resolve, reject) => {
      dbconn.query(
        insertQuery,
        [data.application_id, replaceStatus, replaceStatus, data.remarks],
        (err, result) => {
          if (err) return reject(err);
