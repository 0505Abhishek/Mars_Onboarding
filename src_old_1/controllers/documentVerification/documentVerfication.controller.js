...
const SubmitDocumentVerification = async (req, res) => {
    try {
        ...
        let applicationDetail = await  distributorListModel.getApplicationInfo(applicationId, user_id) 
        ...
        if(data.action=='correction')
        {
            ...
            await insertApplicationHistory(applicationId, user_id, role_name, 5, aseDetail[0].id, aseDetail[0].role, aseDetail[0].employee_name, "", "Correction", "MIS asking for correction");
