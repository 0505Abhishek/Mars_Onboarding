...
const offboardDistributor = async (req, res) => {
  try{
    ...
    let OffboardHierarchy = await offboardModel.getOffboardHierarchy(data.application_id);
    ...
    if(data?.avg_sale == 'no')
    {
      ...
      await offboardModel.insertApprovalWorkflow(OffboardHierarchy[0].application_id, user.id, user.role_id, user);
    }
    ...
    let token = await tokenWebPage.tokenForWebPage(data?.application_id);
    ...
    // await offboardModel.setoffboardingProcessLog(); 
