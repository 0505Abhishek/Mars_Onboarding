...
const offboardList = async (req, res) => {
  try{
        ...
        let allDistributor = await approverModel.getDistributorList(req.cookies.user_id, req.cookies.email);
        ...
        res.render('offboardApprover', { token: navbarviews, notification, data: allDistributor });
  }catch(error){
...
const offboardApplicationViewById = async(req,res)=>{
  ...
  let DistributorData = await approverModel.getDistributorData(application_id);
  return res.render("offboardApprover/distributorView",{token: navbarviews, token: navbarviews, DistributorData:DistributorData[0]});
  }
  catch(error)
  {
  ...
  try {
    ...
    if (actionData.action === 'Approve') {
      ...
      if(NextApprover[0]?.approver_id){
        const userData = await ResignationModel.getUserById(NextApprover[0].approver_id);
        ...
        const userData = await ResignationModel.getUserById(Distributor[0].initiator_id);
        let obj = {};
        obj.sendToEmail = userData?.email_id;
        obj.firmName = Distributor[0]?.firmName;
        obj.userName = userName;
...
module.exports = {
    offboardList,offboardApplicationViewById, SubmitApprover   
}