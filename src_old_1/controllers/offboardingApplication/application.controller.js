...
const offboardList = async (req, res) => {
  try{
        ...
        let allDistributor = await applicationModel.getDistributorList(req.cookies.user_id);
...
const offboardApplicationViewById = async(req,res)=>{
  ...
  let userData = await applicationModel.getUserById(user_id, application_id);
...
const offboardApplicationSubmit = async(req,res)=>{
  ...
  if(applicationData.length==0)
  {
    ...
    await applicationModel.updateoffboardHierarchy(user_id, application_id, 1);
  }
  else if(applicationData[0].approval_count == 2)
  { 
