...
const getDistributorById = async (id, userId) => {
    try {
        return await new Promise((resolve, reject) => {
            const query = `SELECT s_tbl_territory_master.territory_name, prospective_info.* FROM prospective_info
                            ...
                            WHERE prospective_info.id = ? and tbl_approval_workflow.approver_id= ? and tbl_approval_workflow.is_final_approver = 0`;

            dbconn.query(query, [id, userId], (error, results) => {
                if (error) {
                    console.error("Database error:", error);
                    reject(error);
                } else {
