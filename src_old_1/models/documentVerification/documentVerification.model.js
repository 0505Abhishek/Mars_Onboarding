...
const getApplicationInfo = async(applicationId, user_id)=>{
  try {
    const query = `
      SELECT * 
      FROM tbl_approval_workflow 
    ...
    return new Promise((resolve, reject) => {
      dbconn.query(query, [applicationId, user_id], (error, results) => { 
        if (error) {
          console.error("Database error:", error);
          return reject(error);
        }
...
module.exports={getApplicationInfo, getNextApprover, getAseDetail, getUserById, getDistributorList, distributorDetail,approvedByMis, insertCorrectionRecord, updateProspectiveInfo, updateProspectiveInfoMis};